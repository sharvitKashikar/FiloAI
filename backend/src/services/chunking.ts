...
export function chunkText(
  text: string,
  fileName: string,
  chunkSize: number = 1500,  // Increased to keep complete quote sections together
  overlap: number = 300      // Increased overlap for better context continuity
): TextChunk[] {
  const chunks: TextChunk[] = [];
  const CHARS_PER_PAGE = 3000;
  
  // Clean up the text first (important for OCR output)
  ...
  // Ensure we always return at least one chunk if text exists
